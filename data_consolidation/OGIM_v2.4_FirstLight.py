...
from data_consolidation_utils import (read_files_by_keyword, HiddenPrints,
                                      ...
                                      drop_non_oilgas_wells, create_ogim_status_column)
...
from ogim_summary_functions import get_uniques
...
integration_folder = 'OGIM_v2.4_FirstLight\\integrated_results\\'
# final_layers = f'OGIM_v2.4\\layercreation_results_{timestr}\\'
fp_of_output_gpkg = 'C:\\Users\\maobrien\\Environmental Defense Fund - edf.org\\Mark Omara - Infrastructure_Mapping_Project\\Bottom-Up-Infra-Inventory\\OGIM_v2.4_FirstLight\\OGIM_v2.4_FirstLight.gpkg'
excel_report_folder = 'C:\\Users\\maobrien\\Environmental Defense Fund - edf.org\\Mark Omara - Infrastructure_Mapping_Project\\Bottom-Up-Infra-Inventory\\OGIM_v2.4_FirstLight\\'

# check that your desired output paths exist before proceeding
...
excel_reports_by_state_prov = False
...
# =============================================================================
# %% Create US "enhanced" wells layer
# TODO - turn this into a function so it looks less cluttered.
# =============================================================================
# List the states for which we want to DROP HIFLD WELLS because we already have
# state agency wells within `data4geopackage`
drophifld = [
    'ALABAMA',
    'ALASKA',
    'ARKANSAS',
...
# Create a mask that will identify the well records I want to drop,
# then apply the mask to remove these records from `data4geopackage`
mymask = (data4geopackage['Oil_and_Natural_Gas_Wells'].STATE_PROV.isin(drophifld) & (data4geopackage['Oil_and_Natural_Gas_Wells'].SRC_REF_ID == '80'))
data4geopackage['Oil_and_Natural_Gas_Wells'] = data4geopackage['Oil_and_Natural_Gas_Wells'][~mymask]
data4geopackage['Oil_and_Natural_Gas_Wells'] = data4geopackage['Oil_and_Natural_Gas_Wells'].reset_index(drop=True)

# =============================================================================
# %% FIRST LIGHT ONLY: Keep only records that lie in first light countries / states
# In each layer of `data4geopackage`, which is ALL of our integrated OGIM records,
# keep the record if its COUNTRY value contains one of our values of interest,
# OR the STATE_PROV value contains one of our values of interest.
# =============================================================================
get_uniques(data4geopackage, 'COUNTRY')

countries2keep = ['algeria',
                  'australia',
                  'argentina',
                  'bulgaria',
                  'iran',
...
for key in data4geopackage.keys():
    mymask = ((data4geopackage[key].COUNTRY.str.contains('|'.join(countries2keep), case=False)) | (data4geopackage[key].STATE_PROV.str.contains('|'.join(states2keep), case=False)))
    data4geopackage[key] = data4geopackage[key][mymask].reset_index(drop=True)
    print(f'Non-first-light records dropped from {key}')
...
for layername in keylist:
    ...
    # -------------------------------------------------------------------------
    # FILL COUNTRY COLUMN for JUST pipelines, basins, and fields
    if any(x in layername.lower() for x in ['pipeline', 'basin', 'field']):
    ...
    # For all feature types except basins and pipelines
    # if any(x in layername.lower() for x in ['pipeline', 'basin']):
    #     pass
    ...
    if 'offshore' in layername.lower():
        ...
        # Fill the null values with 'N/A'  # FIXME later
